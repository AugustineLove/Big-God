...
const PayrollRun = ({ companyId, periodId }) => {
  ...
  const fetchEligibleStaff = async () => {
    const res = await fetch(`https://susu-pro-backend.onrender.com/payroll/${companyId}/staff/payroll-info`);
    const data = await res.json();
  ...
  const runPayroll = async () => {
    setRunning(true);
    try {
      const res = await fetch(`https://susu-pro-backend.onrender.com/payroll/${companyId}/periods/${periodId}/run`, {
        method: 'POST',
