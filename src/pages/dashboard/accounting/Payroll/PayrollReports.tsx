...
const PayrollReports = ({ companyId }) => {
  ...
  const fetchYearlyReport = async () => {
    const res = await fetch(`http://localhost:5050/api/payroll/${companyId}/reports/yearly?year=${year}`);
  ...
  const exportPAYE = () => {
    window.open(`http://localhost:5050/api/payroll/${companyId}/export/paye?year=${year}`, '_blank');
  };

  const exportSSNIT = () => {
    window.open(`http://localhost:5050/api/payroll/${companyId}/export/ssnit?year=${year}`, '_blank');
  };
