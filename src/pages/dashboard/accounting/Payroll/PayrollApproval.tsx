...
const PayrollApproval = ({ companyId, periodId }) => {
  ...
  const fetchEntries = async () => {
    const res = await fetch(`http://localhost:5050/api/payroll/${companyId}/periods/${periodId}/entries`);
  ...
  const approvePayroll = async () => {
    setApproving(true);
    try {
      const res = await fetch(`http://localhost:5050/api/payroll/${companyId}/periods/${periodId}/approve`, {
        method: 'POST',
  ...
  const markAsPaid = async () => {
    const res = await fetch(`http://localhost:5050/api/payroll/${companyId}/periods/${periodId}/mark-paid`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
