...
const Payslips = ({ companyId, staffId = null }) => {
  ...
  const fetchPayslips = async () => {
    const url = staffId 
      ? `http://localhost:5000/api/payroll/${companyId}/staff/${staffId}/payslips`
      : `http://localhost:5000/api/payroll/${companyId}/payslips`;
  ...
  const downloadPDF = async (payslipId) => {
    const res = await fetch(`http://localhost:5000/api/payroll/${companyId}/payslips/${payslipId}/pdf`);
