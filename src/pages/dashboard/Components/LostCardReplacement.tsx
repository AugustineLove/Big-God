...
const CardReplacementManager = () => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      if (searchType === 'account') {
        url = `https://susu-pro-backend.onrender.com/accounts/search?account_number=${encodeURIComponent(searchValue)}`;
      } else {
        url = `https://susu-pro-backend.onrender.com/customers/search?query=${encodeURIComponent(searchValue)}`;
      }
  ...
  const fetchReplacements = async (id, type) => {
    ...
    try {
      const url = type === 'account' 
        ? `https://susu-pro-backend.onrender.com/accounts/${id}/card-replacements`
        : `https://susu-pro-backend.onrender.com/customers/${id}/card-replacements`;
  ...
  const fetchRecentReplacements = async () => {
    try {
      const response = await fetch('https://susu-pro-backend.onrender.com/card-replacements/recent?limit=10', {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('susupro_token')}`,
          'Content-Type': 'application/json'
        }
  ...
  const handleRequestReplacement = async (e) => {
    ...
    try {
      const response = await fetch(`https://susu-pro-backend.onrender.com/accounts/${formData.account_id}/card/replace-with-record`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('susupro_token')}`,
          'Content-Type': 'application/json'
  ...
  const handleStatusUpdate = async (replacementId, newStatus, additionalData = {}) => {
    try {
      const response = await fetch(`https://susu-pro-backend.onrender.com/card-replacements/${replacementId}/status`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('susupro_token')}`,
          'Content-Type': 'application/json'
  ...
  const fetchCustomerAccounts = async (customerId) => {
    try {
      const response = await fetch(`https://susu-pro-backend.onrender.com/accounts/customer/${customerId}`);
      const result = await response.json();
      if (response.ok) {
