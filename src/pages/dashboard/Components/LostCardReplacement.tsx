...
const CardReplacementManager = () => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      if (searchType === 'account') {
        url = `http://localhost:5050/api/accounts/search?account_number=${encodeURIComponent(searchValue)}`;
      } else {
        url = `http://localhost:5050/api/customers/search?query=${encodeURIComponent(searchValue)}`;
      }
  ...
  const fetchReplacements = async (id, type) => {
    ...
    try {
      const url = type === 'account' 
        ? `http://localhost:5050/api/accounts/${id}/card-replacements`
        : `http://localhost:5050/api/customers/${id}/card-replacements`;
  ...
  const fetchRecentReplacements = async () => {
    try {
      const response = await fetch('http://localhost:5050/api/card-replacements/recent?limit=10', {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('susupro_token')}`,
          'Content-Type': 'application/json'
        }
  ...
  const handleRequestReplacement = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:5050/api/accounts/${formData.account_id}/card/replace-with-record`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('susupro_token')}`,
          'Content-Type': 'application/json'
  ...
  const handleStatusUpdate = async (replacementId, newStatus, additionalData = {}) => {
    try {
      const response = await fetch(`http://localhost:5050/api/card-replacements/${replacementId}/status`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('susupro_token')}`,
          'Content-Type': 'application/json'
  ...
  const fetchCustomerAccounts = async (customerId) => {
    try {
      const response = await fetch(`http://localhost:5050/api/accounts/customer/${customerId}`);
      const result = await response.json();
      if (response.ok) {
