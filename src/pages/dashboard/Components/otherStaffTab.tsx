...
const OtherStaffTab = () => {
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5050/api/staff/${selectedStaff.id}`, {
        method: 'PUT',
        headers: {
  ...
  const handlePermissionsSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5050/api/staff/${selectedStaff.id}/permissions`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
