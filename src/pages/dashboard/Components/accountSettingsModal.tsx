...
const AccountSettingsModal = ({ account, isOpen, onClose, onSave }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/accounts/${accountId}/settings`, {
        method: 'PATCH',
        headers: {
  ...
  const handleReplaceCard = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/accounts/${account.id}/card/replace`, {
        method: 'POST',
  ...
  const handleUnlockAccount = async () => {
    try {
      const res = await fetch(`http://localhost:5000/api/accounts/${account.id}/unlock`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${localStorage.getItem('susupro_token')}` },
      });
      if (!res.ok) throw new Error();
