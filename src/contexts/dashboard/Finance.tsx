...
type FinanceContextType ={
  ...
  reduceBudget: (company_id: string, data: Omit<Budget, "id">) => Promise<void>;
};
...
export const FinanceProvider = ({ children }: { children: ReactNode }) => {
  ...
  const fetchFinanceData = async (range = 'this-month', startDate?: string, endDate?: string) => {
    try{
      ...
      const res = await fetch(`http://localhost:5000/api/financials/get-financials/${companyId}?${params}`);
  ...
  const addBudget = async ( company_id: string, data: Omit<Budget, "id">) => {
    try {
      ...
      const res = await fetch(`http://localhost:5000/api/financials/budget`, { 
        method: "POST", 
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
        company_id,
  ...
  const reduceBudget = async ( company_id: string, data: Omit<Budget, "id">) => {
    try {
      setLoading(true);
      const recorded_by = userUUID;
  ...
  return (
    <FinanceContext.Provider
      value={{ loading, error, data, fetchFinanceData, addAsset, addExpense, addPayment, addBudget, reduceBudget }}
    >
      {children}
    </FinanceContext.Provider>
  );
