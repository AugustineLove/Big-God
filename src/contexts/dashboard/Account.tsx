...
export const AccountsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  try {
    ...
    const res = await fetch(
      `https://susu-pro-backend.onrender.com/loans/?company_id=${companyId}`
    );
  ...
  const fetchAccounts = async (customerId: string) => {
    setLoading(true);
    try {
        const res = await fetch(`https://susu-pro-backend.onrender.com/accounts/customer/${customerId}`);
      if (!res.ok) {
        const errorText = await res.text();
        return;
      }
  ...
  try {
    const res = await fetch(
      `https://susu-pro-backend.onrender.com/customers/account/${accountNumber}`
    );
  ...
  const getAllCompanyAccounts = async () => {
    setLoading(true);
    try {
        const res = await fetch(`https://susu-pro-backend.onrender.com/accounts/company/${companyId}`);
      if (!res.ok) {
        const errorText = await res.text();
  ...
  const addAccount = async(newAccount: Omit<Account, 'id' | 'created_at'>)=>{
    ...
    const res = await fetch(
      `https://susu-pro-backend.onrender.com/accounts/${accountId}/toggle-status`,
      {
        method: "PATCH",
        headers: {
