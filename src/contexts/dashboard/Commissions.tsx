...
export const CommissionStatsProvider: React.FC<Props> = ({
  ...
  const refreshCommissionStats = async () => {
    try {
      setLoading(true);
      const res = await fetch(`https://susu-pro-backend.onrender.com/commissions/stats/${companyId}`);
  ...
  const fetchCommissions = async () => {
    try {
      setLoading(true);
      const res = await fetch(`https://susu-pro-backend.onrender.com/commissions/all/${companyId}`);
